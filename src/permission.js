...
const whiteList = ['/', '/home', '/exams','/exam-details', '/courses','/course-details', '/register', '/static', '/login', '/wxLogin', '/auth-redirect', '/404', '/401', '/lock', '/reset-password']// no redirect whitelist
...
router.beforeEach((to, from, next) => {
  ...
  if (to.path.indexOf('/mobile') !== -1) {
    ...
    if (whiteList.indexOf(to.path) !== -1) { // 在免登录白名单，直接进入
      ...
      if(isWeixin()){
        next(`/wxLogin?redirect=${to.fullPath}`) // 否则全部重定向到登录页
      }else{
        //next('/login') // 重定向到登录页
